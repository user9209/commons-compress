...
final class ChaCha20Decoder extends AbstractCoder {

    private static final class ChaChaDecoderInputStream extends InputStream {
        ...
        private ChaChaDecoderInputStream(final InputStream in, final Coder coder, final String archiveName, final byte[] passwordBytes) {
            this.in = in;
            this.coder = coder;
        ...
        private CipherInputStream init() throws IOException {
            ...
            if (coder.properties == null) {
                throw new IOException("Missing ChaCha20 properties in " + archiveName);
            }
            if (coder.properties.length < 2) {
                throw new IOException("ChaCha20 properties too short in " + archiveName);
            }
            ...
            if (numCyclesPower == 0x3f) {
                ...
                System.arraycopy(salt, 0, keyBytes, 0, saltSize);
                System.arraycopy(passwordBytes, 0, keyBytes, saltSize, Math.min(passwordBytes.length, keyBytes.length - saltSize));
            } else {
                keyBytes = ChaCha20Options.kdf(passwordBytes, numCyclesPower, salt);
            }

            final SecretKey chachaKey = ChaCha20Options.newSecretKeySpec(keyBytes);
            try {
                final Cipher cipher = Cipher.getInstance(ChaCha20Options.TRANSFORMATION);
    ...
    private static final class ChaCha20Poly1305DecoderOutputStream extends OutputStream {
        private final CipherOutputStream cipherOutputStream;
        private final int cipherBlockSize;
        private final byte[] cipherBlockBuffer;
        private int count;

        private ChaCha20Poly1305DecoderOutputStream(final ChaCha20Options opts, final OutputStream out) {
    ...
    InputStream decode(final String archiveName, final InputStream in, final long uncompressedLength, final Coder coder, final byte[] passwordBytes,
        ...
        return new ChaChaDecoderInputStream(in, coder, archiveName, passwordBytes);
    }

    @Override
    OutputStream encode(final OutputStream out, final Object options) throws IOException {
        return new ChaCha20Poly1305DecoderOutputStream((ChaCha20Options) options, out);
    }

    @Override
    byte[] getOptionsAsProperties(final Object options) throws IOException {
        final ChaCha20Options opts = (ChaCha20Options) options;
